...
const AddImageButton: React.FC<AddImageButtonProps> = ({
  ...
  const onAddImage = useCallback(async (imageSrc: string) => {
    ...
    try {
      ...
      if (response.data && response.data.galleryImages) {
        // Assuming the API returns the updated user object with the new gallery images
        const newImage = response.data.galleryImages[0]; // Get the most recently added image
  ...
  }, [currentUser, onImageAdded, profileGalleryModal]);
