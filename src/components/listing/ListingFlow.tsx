...
import ImagesListStep from './steps/ImagesListStep';
import ImageFormStep from './steps/ImageFormStep';
...
export default function ListingFlow() {
  ...
  const [storeHours, setStoreHours] = useState<StoreHourType[]>(initialStoreHours);
  ...
  const getNextStep = useCallback((): STEPS | null => {
    // Handle special cases for form steps
    if (step === STEPS.SERVICES_FORM) return STEPS.SERVICES_LIST;
    if (step === STEPS.IMAGES_FORM) return STEPS.IMAGES_LIST;
  ...
  const canProceed = useCallback((): boolean => {
    switch (step) {
      ...
      case STEPS.IMAGES_LIST:
        return Boolean(imageSrc); // Need at least main image
      case STEPS.IMAGES_FORM:
        return true;
  ...
  const isFormStep = step === STEPS.SERVICES_FORM || step === STEPS.IMAGES_FORM;
        ...
        return (
          <LocationStep
            ...
            onAddressSelect={(data) => {
              ...
              setCustomValue('location', `${data.city}, ${data.state}`);
              clearErrors(['address', 'zipCode', 'location']);
            }}
            onFieldChange={(fieldId) => clearErrors(fieldId)}
            errors={errors}
          />
        );
      case STEPS.SERVICES_LIST:
