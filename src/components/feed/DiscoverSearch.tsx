...
const DiscoverSearch: React.FC<DiscoverSearchProps> = ({
  ...
  const containerClasses = isHeroMode
    ...
    : "bg-neutral-100/80 backdrop-blur-sm border border-neutral-200/60 rounded-xl";

  const iconButtonClasses = isHeroMode
    ? "p-2.5 rounded-lg hover:bg-white/10 text-white/80 hover:text-white transition-all duration-200"
    : "p-2.5 rounded-lg hover:bg-white text-neutral-600 hover:text-neutral-900 transition-all duration-200";

  return (
    <div className={`${containerClasses} px-3 py-2 flex items-center gap-2 mt-3`}>
      <button
