...
const Post: React.FC<PostProps> = ({ post, currentUser, categories }) => {
  ...
  const handleDelete = async (e: React.MouseEvent) => {  // Add event parameter
  ...
  const handleNotInterested = useCallback(async (e: React.MouseEvent) => {
    ...
    try {
      console.log('Sending hide request for post:', post.id); // Add this
      const response = await axios.post(`/api/postActions/${post.id}/hide`);
      console.log('Hide response:', response.data); // Add this
      setIsHidden(true);
