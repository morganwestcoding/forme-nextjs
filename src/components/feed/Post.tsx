...
const Post: React.FC<PostProps> = ({ post, currentUser, categories }) => {
        ...
        <>
         <div className='w-full h-auto rounded-lg shadow-sm bg-[#ffffff] p-6 md:mr-8 my-4 relative cursor-pointer' onClick={openModal}>
            ...
            <div className="flex items-center">     
              ...
              <div className="ml-3 flex flex-col">
                <div className="flex items-center pb-1">
                  ...
                  <span className="text-sm text-[#6B7280]">&middot; {formattedDate || 'Loading time...'}</span>
                </div>
                <div className={`flex text-sm items-center ${post.location ? 'text-gray-600' : '-ml-2'}`}>
                  {post.location && (
                    <span>{city}, {stateAcronym}</span>
                  )}
                  <span className={`ml-2 py-1.5 px-3 rounded text-white shadow-sm shadow-slate-300 flex items-center justify-center text-xs ${categoryColor}`}>
                    {post.category}
                  </span>
                </div>
              </div>
            ...
            <div className="mt-3 relative">
              ...
              <div className="bottom-0 left-0 flex space-x-2.5 p-2 -ml-2 -mb-4 mt-1.5">
              <div className="flex items-center justify-center bg-slate-50  rounded-full p-3 cursor-pointer shadow-sm shadow-slate-300  ">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width={19} height={19} color={"#6B7280"} fill={"none"}>
    ...
    {/* Lines on top with gray color */}
    <path d="M8 13.5H16M8 8.5H12" stroke="#6B7280" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
</div>
<div 
  onClick={(e) => { e.stopPropagation(); handleLike(); }}
  className="flex items-center justify-center bg-slate-50 shadow-sm shadow-slate-300 rounded-full p-3 cursor-pointer relative">  {/* Added 'relative' here */}
  <svg 
    xmlns="http://www.w3.org/2000/svg" 
    viewBox="0 0 24 24" 
    width={19} 
    height={19} 
    color={isLiked ? "#6B7280" : "#6B7280"} 
    fill={isLiked ? "#E2E8F0" : "#ffffff"}
  >
  ...
  {likes.length > 0 && (
    <div className="absolute -top-1 -right-2 bg-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-thin text-[#6B7280] shadow">
      {likes.length}
    </div>
  )}
</div>
                <div
                  ...
                  className="flex items-center bg-slate-50  justify-center shadow-sm shadow-slate-300  rounded-full p-3 cursor-pointer relative">
                  <svg 
                    xmlns="http://www.w3.org/2000/svg" 
                    viewBox="0 0 24 24" 
                    width={19} 
                    height={19} 
                    color={isBookmarked ? "#6B7280" : "#6B7280"} 
                    fill={isBookmarked ? "#E2E8F0" : "#ffffff"}
                  >
