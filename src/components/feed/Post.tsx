...
const Post: React.FC<PostProps> = ({ post, currentUser, categories }) => {
  ...
  const categoryColor = categories.find(cat => cat.label === post.category)?.color || 'bg-[#5FBFF9]';
  const badgeColor = categoryColor.replace('bg-[', '').replace(']', '') || '#5FBFF9';
        ...
        <>
         <div className='w-full h-auto rounded-lg shadow-sm bg-[#ffffff] p-6 md:mr-6 my-4 relative cursor-pointer' onClick={openModal}>
            ...
            <div className="mt-3 relative">
              ...
              <div className="flex items-center justify-center bg-gray-50 border border-[#71717A] rounded-full p-3 cursor-pointer   ">
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width={21} height={21} color={"#71717A"} fill={"none"}>
    ...
    <path d="M8 13.5H16M8 8.5H12" stroke="#71717A" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
</div>
<div 
  onClick={(e) => { e.stopPropagation(); handleLike(); }}
  className="flex items-center justify-center bg-gray-50 border border-[#71717A] rounded-full p-3 cursor-pointer relative">  {/* Added 'relative' here */}
  <svg 
    xmlns="http://www.w3.org/2000/svg" 
    viewBox="0 0 24 24" 
    width={21} 
    height={21} 
    color={isLiked ? "#71717A" : "#71717A"} 
  ...
  {likes.length > 0 && (
    <div className="absolute -top-1 -right-2 bg-[#5FBFF9] rounded-full w-5 h-5 flex items-center justify-center text-xs font-thin text-white shadow">
      {likes.length}
    </div>
  )}
</div>
                <div
                  ...
                  className="flex items-center bg-gray-50 border border-neutral-500 justify-center   rounded-full p-3 cursor-pointer relative">
                  <svg 
                    xmlns="http://www.w3.org/2000/svg" 
                    viewBox="0 0 24 24" 
                    width={21} 
                    height={21} 
                    color={isBookmarked ? "#71717A" : "#71717A"} 
