...
onImageChange,
    ...
    // Handler for the Cloudinary upload widget
    const handleUpload = (result: any) => {
        if (result.event === "success") {
          const newImageUrl = result.info.secure_url;
          onImageChange(newImageUrl);
          useUserStore.getState().updateUserImage(newImageUrl); // Directly update the store
        }
      };
...
<div className="flex justify-between w-full mt-8 px-20">
    ...
    <div className="relative text-white text-center h-56 py-8 w-full rounded-lg flex justify-center items-center">
      ...
      <div className="w-5/12 h-40 bg-white bg-opacity-95 rounded-lg flex shadow-md z-50">
        ...
        <Image
                    src={user.image || "/people/rooster.jpg"}
                    alt="User Avatar"
                    layout="fill"
                    objectFit="cover"
            ...
            <span className="text-gray-800 font-semibold ml-3 justify-start">{user?.name}</span>
