...
const PostModal = () => {
  ...
  const { isOpen, post, comments, currentUser, onClose, onLike, onBookmark, refreshComments } = postModal;
  ...
  // Update formatted date when post changes
  useEffect(() => {
    if (isOpen && post) {
  ...
  }, [isOpen, post, formatDate]);

  // Call refreshComments when modal opens
  useEffect(() => {
    if (isOpen && post) {
      refreshComments();
    }
  }, [isOpen, post, refreshComments]);

  // If modal is not open or no post data, render nothing
  if (!isOpen || !post) {
    return null;
  }
  ...
  const handleSubmitComment = async () => {
    if (!comment.trim() || isSubmitting || !post) return;
    if (!currentUser) {
    ...
    try {
      ...
      postModal.setComments([newComment, ...comments]);
  ...
  const renderMedia = () => {
    if (post.imageSrc) {
      return (
        <div className="rounded-lg overflow-hidden relative w-full mb-4 bg-black">
  ...
  const bodyContent = (
    <div className="flex flex-col h-full bg-white">
      {/* Main content container with two main sections */}
      <div className="flex flex-col max-h-[80vh]">
        ...
        <div className="overflow-y-auto flex-1 pb-4">
          ...
          {/* Post content */}
          <p className='text-sm mb-3 text-[#000000]'>{post.content}</p>
            ...
            <div className="overflow-y-auto max-h-[30vh]">
              {comments && comments.length > 0 ? (
                comments.map((comment) => (
                  <div key={comment.id} className="flex items-start mb-3 last:mb-0 bg-gray-50 p-3 rounded-lg">
        ...
        <div className="sticky bottom-0 bg-white pt-2 border-t border-gray-200 -mx-6 mt-auto">
  ...
  return (
    <Modal
      id="post-modal"
      modalContentId="post-modal-content"
      isOpen={isOpen}
      onClose={onClose}
      onSubmit={() => {}}
      body={bodyContent}
