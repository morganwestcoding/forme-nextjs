        ...
        <div className="flex flex-col items-center w-full">
          ...
          <ul className="list-none m-0 p-0 flex flex-col items-center hover:text-white">
            {menuItems.map((item) => (
              <li
                key={item.label}
                className={`group flex items-center justify-start mb-3 p-2 rounded-lg border ${
                  selectedButton === item.label.toLowerCase() ? 'bg-[#e2e8f0]' : 'bg-[#ffffff] hover:bg-[#e2e8f0]'
                } w-36`}
                onClick={() => {
                  router.push(item.route);
                  setSelectedButton(item.label.toLowerCase());
                }}
              >
                ...
                <span className={`ml-5 text-xs font-light ${
                  selectedButton === item.label.toLowerCase() ? 'text-white' : 'text-[#a2a2a2] group-hover:text-white'
                }`}>{item.label}</span>
              </li>
            ))}
          </ul>
