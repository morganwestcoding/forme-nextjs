...
const SmartBadgeRating: React.FC<SmartBadgeRatingProps> = ({
  ...
  const getStatusTextColor = (color: string) => {
    switch (color) {
      ...
      case 'orange': return 'text-yellow-300';
  ...
  const getStatusBorderColor = (color: string) => {
    switch (color) {
      case 'green': return 'border-white/20 hover:border-green-300/60';
      case 'orange': return 'border-white/20 hover:border-yellow-300/60';
      case 'red': return 'border-white/20 hover:border-rose-400/60';
      default: return 'border-white/20 hover:border-white/60';
    }
  };
  ...
  return (
    <div className="flex items-center gap-2">
      {/* Rating Button */}
      <div className="group/rating border border-white/20 rounded-lg px-3 py-2 bg-white/10 backdrop-blur hover:border-white/60 hover:bg-white/20 transition-colors duration-200">
      ...
      <button
        ...
        className={`${getStatusTextColor(timeStatus.color)} border ${getStatusBorderColor(timeStatus.color)} rounded-lg px-3.5 py-2 bg-white/10 backdrop-blur hover:bg-white/20 transition-colors duration-200 text-xs`}
        type="button"
      >
        {timeStatus.message}
      </button>
