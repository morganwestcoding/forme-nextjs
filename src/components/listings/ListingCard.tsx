  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        <Image src={cardImage} alt={data.title} fill className="w-full h-full object-cover" />
        <div className="absolute inset-0 card__overlay" />
      </div>

      <div className="relative z-10">
        <div className="relative h-[345px] overflow-hidden">
          {/* Category chip (kept small) */}
          ...
          <div className="absolute bottom-5 left-5 right-5 text-white z-20">
            <div className="flex items-center gap-1.5 mb-0.5">
              <h1 className="text-[20px] leading-6 font-semibold drop-shadow-lg">
                {data.title}
              </h1>
              {/* verified icon */}
              <svg
            ...
            <p className="text-[11px] leading-4 drop-shadow-md font-light flex items-center mb-1.5">
              {city}
              {state ? `, ${state}` : ''} • 2.3 miles away
            </p>

            {/* Compact row: rating + open status only */}
            <SmartBadgeRating
