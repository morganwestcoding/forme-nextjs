...
const ShopCard: React.FC<ShopCardProps> = ({
  ...
  // Main navigation handler for the entire card
  ...
  const handleVisitShop = (e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent card navigation (though both go to same place)
    router.push(`/shops/${data.id}`);
  };

  // Handle SmartBadge clicks
  const handleRatingClick = (e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent card navigation
    // Could navigate to shop page with reviews section
    router.push(`/shops/${data.id}#reviews`);
  };

  const handleFollowerClick = (e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent card navigation
    // Could show followers modal or navigate to followers page
