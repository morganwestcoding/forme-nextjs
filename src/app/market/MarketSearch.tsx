...
const MarketSearch: React.FC<MarketSearchProps> = ({ isHeroMode = false, categoryNav, basePath = '/market', onCreateClick }) => {
  ...
  const iconButtonClasses = isHeroMode
    ? "p-1.5 sm:p-2 rounded-lg sm:rounded-xl hover:bg-white/10 text-white/80 hover:text-white transition-all duration-200"
    : "p-1.5 sm:p-2 rounded-lg sm:rounded-xl hover:bg-neutral-200 text-neutral-600 hover:text-neutral-900 transition-all duration-200";
      ...
      <div className={`${containerClasses} mt-3 overflow-hidden`}>
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5">
          <input
            ...
            className={`flex-1 text-[13px] sm:text-[14px] bg-transparent border-none outline-none font-normal pl-2 sm:pl-3 ${isHeroMode ? 'text-white placeholder-white/50' : 'text-neutral-900 placeholder-neutral-400'}`}
          />
