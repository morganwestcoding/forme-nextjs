...
interface PageSearchProps {
  ...
  placeholder?: string;
  /** Filter types for contextual search */
  filterTypes?: Array<'listing' | 'shop' | 'service' | 'employee' | 'user' | 'post'>;
  /** Custom action buttons to render in the search bar */
  actionButtons?: React.ReactNode;
  /** Whether to show the default action buttons (attach, filters, create) */
  showDefaultActions?: boolean;
  /** Custom handler for create button click */
  onCreateClick?: () => void;
  ...
  showCreate?: boolean;
  /** Entity ID for contextual/local filtering */
  entityId?: string;
  /** Entity type for contextual/local filtering */
...
const PageSearch: React.FC<PageSearchProps> = ({
  ...
  const iconButtonClasses = "p-2 rounded-xl text-stone-400 dark:text-stone-500 hover:text-stone-600 dark:hover:text-white hover:bg-stone-100 dark:bg-stone-800 dark:hover:bg-stone-800 transition-colors";
    ...
    <>
      ...
      {showFilters && (
        <button
          ...
          title="Filters"
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-5 h-5 sm:w-[22px] sm:h-[22px]" color="currentColor" fill="none">
      ...
      {isDropdownOpen && (
        <FloatingPortal>
          <div
            ...
            className="z-[50] bg-white dark:bg-stone-800 border border-stone-100 dark:border-stone-700/50 rounded-xl shadow-elevation-2 p-1.5 w-[160px]"
            style={{
              ...floatingStyles,
          ...
          >
            {actions.map((item) => (
              <button
                ...
                className="w-full px-3 py-2.5 rounded-xl hover:bg-stone-50 dark:bg-stone-900 dark:hover:bg-stone-700 transition-colors duration-150 text-left text-[13px] font-medium text-stone-700 dark:text-stone-200"
                type="button"
              >
                {item.label}
              </button>
