  ...
  return (
    <div className={`${center ? 'text-center' : 'text-left'} space-y-1.5`}>
      <div className="text-xl font-semibold tracking-tight flex items-center justify-start text-stone-900 dark:text-white">
        {title}
        {label && category && (
          <div className={`rounded-full px-3 py-1.5 ml-3 text-xs font-medium shadow-elevation-1 ${category ? category.color : defaultBgColor} text-white inline-flex items-center`}>
            {label}
          </div>
        )}
      </div>
