              ...
              <button
                ...
                className={`aspect-square p-4 rounded-xl border-2 transition-all duration-200 flex flex-col items-center justify-center text-center hover:shadow-md ${
                  selectedEmployee?.value === emp.value
                    ? 'border-[#60A5FA] bg-blue-50 shadow-md'
                    : 'border-gray-200 hover:border-gray-300 bg-white'
                }`}
              >
                <div className="flex flex-col items-center gap-3">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-semibold text-lg ${
                    selectedEmployee?.value === emp.value ? 'bg-[#60A5FA]' : 'bg-gray-400'
                  }`}>
                    {emp.label.split(' ').map(n => n[0]).join('').toUpperCase()}
                  </div>
                  <span className={`text-sm font-medium text-center leading-tight ${
                    selectedEmployee?.value === emp.value ? 'text-[#60A5FA]' : 'text-gray-900'
                  }`}>
                    {emp.label}
                  </span>
                </div>
              ...
              {serviceOptions.map(service => (
                <button
                  ...
                  className={`aspect-square p-4 rounded-xl border-2 transition-all duration-200 flex flex-col items-center justify-center text-center hover:shadow-md relative ${
                    isServiceSelected(service.value) ? 'border-[#60A5FA] bg-blue-50 shadow-md' : 'border-gray-200 hover:border-gray-300 bg-white'
                  }`}
                >
                  <div className={`absolute top-2 right-2 w-5 h-5 rounded border-2 flex items-center justify-center transition-all duration-200 ${
                    isServiceSelected(service.value) ? 'bg-[#60A5FA] border-[#60A5FA]' : 'border-gray-300 bg-white'
                  }`}>
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      isServiceSelected(service.value) ? 'bg-blue-100 text-[#60A5FA]' : 'bg-gray-100 text-gray-600'
                    }`}>
                    ...
                    <div className="flex flex-col gap-1">
                      <span className={`text-sm font-medium ${isServiceSelected(service.value) ? 'text-[#60A5FA]' : 'text-gray-900'}`}>
                        {service.label.split(' - ')[0]}
                      </span>
                      <span className={`text-xs font-semibold ${isServiceSelected(service.value) ? 'text-[#60A5FA]' : 'text-gray-600'}`}>
                        ${service.price}
                      </span>
                    </div>
                  </div>
            ...
            <div className="bg-blue-50 rounded-xl p-4 border border-blue-200">
              <div className="flex justify-between items-center">
                ...
                <div className="text-right">
                  <p className="text-lg font-bold text-[#60A5FA]">${totalPrice}</p>
                </div>
              </div>
            </div>
          )}
              ...
              <button
                ...
                className={`aspect-square p-3 rounded-xl border-2 transition-all duration-200 flex items-center justify-center ${
                  isDisabled
                    ...
                    : isSelected
                      ? 'border-[#60A5FA] bg-blue-50 text-[#60A5FA] shadow-md'
    ...
    bodyContent = (
      <div className="flex flex-col gap-6">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="border-t border-gray-300 pt-2 mt-3">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-lg text-[#60A5FA]">${totalPrice}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <textarea
          ...
          className="w-full p-4 rounded-xl bg-white text-gray-700 border-2 border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#60A5FA] focus:border-[#60A5FA] transition-all duration-200"
          rows={3}
        />
      </div>
    );
