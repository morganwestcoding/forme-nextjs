...
const Sidebar: React.FC<SidebarProps> = ({ currentUser }) => {
  ...
  const navItems: { label: string; href: string; active: boolean; onClick?: () => void }[] = [
    ...
    ...(currentUser ? [
      { label: "Bookings", href: "/bookings/reservations", active: isActive("/bookings/reservations", ["/bookings"]) },
    ] : []),
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6 h-full p-4 pt-12">
        {/* Logo */}
        <div
          style={{
            opacity: hasAnimated ? 1 : 0,
        ...
        >
          ...
          {!isCollapsed && phraseIndex !== null && (
            <p
              className="mt-3 text-sm tracking-wide text-zinc-500"
              style={{
                opacity: hasAnimated ? 1 : 0,
        ...
        >
          <DropdownMenu open={isUserMenuOpen} onOpenChange={setIsUserMenuOpen}>
            <DropdownMenuTrigger className="w-full outline-none">
              <div className={`group flex items-center gap-3 transition-all duration-200 cursor-pointer ${isCollapsed ? "justify-center" : ""}`}>
            ...
            {navItems.map((item, i) => {
              ...
              return (
                <El
                  ...
                  className="group flex items-center py-2.5 text-[14px]"
                >
                  {!isCollapsed && (
        ...
        {isCollapsed && (
          <button onClick={toggle} className="text-zinc-400 hover:text-zinc-600">
