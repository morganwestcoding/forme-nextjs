...
export default async function AdminDisputesPage() {
  ...
  const statusStyles: Record<string, string> = {
    ...
    lost: 'text-stone-700 bg-stone-50 ring-stone-200',
  };
    ...
    <Container>
      <div className="mt-8 mb-12">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-semibold text-stone-900 tracking-tight">Disputes</h1>
          <p className="text-[14px] text-stone-400 mt-1">{disputes.length} disputes</p>
        </div>
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-stone-200/60 bg-stone-50/50">
                  ...
                  <th className="text-left py-3 px-4 font-medium text-stone-500">Reason</th>
                  <th className="text-left py-3 px-4 font-medium text-stone-500">Service</th>
                  <th className="text-left py-3 px-4 font-medium text-stone-500">Customer</th>
              ...
              <tbody>
                {disputes.map((dispute) => {
                  ...
                  return (
                    <tr key={dispute.id} className="border-b border-stone-100 hover:bg-stone-50/50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-stone-600">{dispute.reason || '—'}</td>
                      <td className="py-3 px-4 text-stone-600">
                        {reservation?.serviceName || '—'}
                      </td>
                      <td className="py-3 px-4">
                        <div>
                          <p className="text-stone-900">{reservation?.user?.name || '—'}</p>
                          <p className="text-[11px] text-stone-400">{reservation?.user?.email || ''}</p>
                        </div>
                      </td>
                      <td className="py-3 px-4 text-stone-600">
