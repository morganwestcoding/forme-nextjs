...
const TeamClient: React.FC<TeamClientProps> = ({ currentUser }) => {
  ...
  const canSeeRevenue = isOwnerOfSelected || myEmployee?.teamRole === 'manager';
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                ...(canSeeRevenue ? [{ label: 'This Month', value: `$${stats.monthRevenue.toLocaleString()}` }] : []),
              ].map((stat, i) => (
                <div
                  key={i}
            ...
            {pendingTimeOffRequests.length > 0 && (
              <div>
                <h2 className="text-lg font-semibold text-stone-900 dark:text-stone-100 tracking-tight mb-4">
                  Time Off Requests
                  <span className="ml-2 text-[12px] font-normal text-warning">{pendingTimeOffRequests.length} pending</span>
                </h2>
                <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                  {[
                    ...
                    <div key={i} className="text-center">
                      ...
                      <p className={`text-[22px] font-bold tabular-nums mt-1 ${stat.highlight ? 'text-danger' : 'text-stone-900 dark:text-stone-100'}`}>{stat.value}</p>
                    </div>
                  ))}
                </div>
                      ...
                      <div className="p-5 border-b border-stone-100 dark:border-stone-800">
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                          {[
                            ...
                            <div key={i} className="text-center">
                              <p className="text-[11px] text-stone-400 dark:text-stone-500">{stat.label}</p>
                              <p className={`text-[18px] font-bold tabular-nums mt-0.5 ${stat.highlight ? 'text-danger' : 'text-stone-900 dark:text-stone-100'}`}>{stat.value}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                                    ...
                                    {waivingPeriodId === period.id ? (
                                      <div className="flex items-center gap-1.5">
                                        <input
                                          ...
                                          autoFocus
                                        />
                                        <button onClick={() => waivePeriod(period.id, member.id, waiveReason)} className="text-[11px] text-success-soft-foreground font-medium">OK</button>
                                    ...
                                    {denyingPayoutId === payout.id ? (
                                      <div className="flex items-center gap-1.5">
                                        <input
                                          ...
                                          autoFocus
                                        />
                                        <button onClick={() => handlePayoutAction(payout.id, 'deny', denyNote)} className="text-[11px] text-danger-soft-foreground font-medium">Deny</button>
