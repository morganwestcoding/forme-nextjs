      ...
      <div className="flex gap-6 -mx-6 md:-mx-24 px-6 md:px-24 -mt-2 md:-mt-8 md:h-[calc(100vh-2rem)] md:overflow-hidden">
        ...
        <div ref={leftColumnRef} className="w-[320px] flex-shrink-0 hidden md:flex flex-col gap-4 py-10">
          ...
          >
            ...
            <div className="pt-8 pb-5 px-6 text-center relative">
              ...
              <div className="mt-3">
                ...
                <div className="flex items-center justify-center gap-2 mt-1">
                  ...
                  {discount > 0 && (
                    <span className="text-xs font-medium text-rose-500">{discount}% off</span>
                  )}
                </div>

                {!inStock && (
                  <p className="text-xs text-danger font-medium mt-1">Sold out</p>
                )}
              </div>
            </div>
            ...
            <div className="px-6 py-5">
              <div className="flex items-center justify-between text-center">
                <div className="flex-1">
                  <p className="text-lg font-bold text-stone-900 dark:text-stone-100 tabular-nums">{product.inventory}</p>
                  <p className="text-xs text-stone-400 dark:text-stone-500 mt-0.5">in stock</p>
                </div>
                <div className="w-px h-10 bg-stone-100 dark:bg-stone-800" />
                <div className="flex-1">
                  <p className="text-lg font-bold text-stone-900 dark:text-stone-100 tabular-nums">{product.favoritedBy?.length || 0}</p>
                  <p className="text-xs text-stone-400 dark:text-stone-500 mt-0.5">likes</p>
                </div>
                <div className="w-px h-10 bg-stone-100 dark:bg-stone-800" />
                <div className="flex-1">
                  <p className="text-lg font-bold text-stone-900 dark:text-stone-100 tabular-nums">{product.reviews?.length || 0}</p>
            ...
            <div className="px-6 py-5">
              {product.description ? (
                <p className="text-sm text-stone-500  dark:text-stone-500 leading-[1.7] line-clamp-4">
                  {product.description}
                </p>
              ) : (
                <p className="text-sm text-stone-400 dark:text-stone-500 leading-[1.7]">
          ...
          <div className="space-y-12">
            ...
            <section>
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-xs font-medium text-stone-500  dark:text-stone-500 bg-stone-100 dark:bg-stone-800 px-2.5 py-1 rounded-full tabular-nums">{allImages.length}</span>
                </div>
              </div>
              ...
              <section>
                ...
                {options.map((option: any, optIdx: number) => (
                  <div key={optIdx} className="mb-4">
                    <p className="text-xs font-medium text-stone-600 dark:text-stone-300 mb-3">{option.name}</p>
            ...
            <section>
              ...
              <div className="inline-flex items-center border border-stone-200 dark:border-stone-800 rounded-xl overflow-hidden">
                ...
                <span className="w-12 h-12 flex items-center justify-center text-sm font-semibold text-stone-900 dark:text-stone-100 tabular-nums border-x border-stone-200 dark:border-stone-800">
                  {quantity}
                </span>
                <button
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-4 max-w-lg">
                {product.category?.name && (
                  <div>
                    <p className="text-xs text-stone-400 dark:text-stone-500 uppercase tracking-wider">Category</p>
                    <p className="text-sm text-stone-700 dark:text-stone-200 font-medium mt-0.5">{product.category.name}</p>
                  </div>
                )}
                {product.sku && (
                  <div>
                    <p className="text-xs text-stone-400 dark:text-stone-500 uppercase tracking-wider">SKU</p>
                    <p className="text-sm text-stone-700 dark:text-stone-200 font-medium mt-0.5">{product.sku}</p>
                  </div>
                )}
                {product.weight && (
                  <div>
                    <p className="text-xs text-stone-400 dark:text-stone-500 uppercase tracking-wider">Weight</p>
                    <p className="text-sm text-stone-700 dark:text-stone-200 font-medium mt-0.5">{product.weight}g</p>
                  </div>
                )}
                <div>
                  <p className="text-xs text-stone-400 dark:text-stone-500 uppercase tracking-wider">Availability</p>
                  <p className={`text-sm font-medium mt-0.5 ${inStock ? 'text-success-soft-foreground' : 'text-danger'}`}>
              ...
              {product.tags && product.tags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-5">
                  {product.tags.map((tag, idx) => (
                    <span
                      key={idx}
                      className="text-xs font-medium text-stone-500  dark:text-stone-500 bg-stone-100 dark:bg-stone-800 px-2.5 py-1 rounded-full"
                    >
                      {tag}
                    </span>
                  ))}
            ...
            {product.shop && (
              <section>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-stone-900 dark:text-stone-100 truncate">{product.shop.name}</p>
                    {product.shop.location && (
                      <p className="text-xs text-stone-400 dark:text-stone-500 truncate">{product.shop.location}</p>
                    )}
                  </div>
              ...
              <section>
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-xs font-medium text-stone-500  dark:text-stone-500 bg-stone-100 dark:bg-stone-800 px-2.5 py-1 rounded-full tabular-nums">{relatedProducts.length}</span>
                  </div>
                </div>
